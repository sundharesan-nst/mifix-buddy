...
function getLevelResponse(level, query, fast, slow, groq, leng = "", response = "") {
...
.then((data) => {
    ...
    streamEventSource.onmessage = function(event) {
      ...
      const streamData = JSON.parse(event.data); // Adjust this based on your data format
        // console.log("data: ",streamData.response)
...
// function getLevelResponse(level, query, fast, slow, groq, leng = "") {
//   const formData = new FormData();
//   formData.append("query", query);
//   formData.append("fast", fast);
//   formData.append("leng", leng);
...
function updateBox(level, data, query, fast, leng = "") {
  ...
  {
  if (level === "level1") {
    console.log("Appending nuttons for L2")
    appendButton(2, processedResponse, query, fast, slow, groq, leng); 
  } else if (level === "level2") {
    console.log("Appending nuttons for L3")
...
function handleSubmitButton(query, fast, slow, groq, leng = "") {
  ...
  getLevelResponse("level1", query, fast, slow, groq, leng);
  msgerInput.value = "";
}
